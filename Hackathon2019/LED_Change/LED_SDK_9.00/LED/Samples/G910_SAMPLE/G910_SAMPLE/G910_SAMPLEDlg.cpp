...
BEGIN_MESSAGE_MAP(CG910_SAMPLEDlg, CDialogEx)
    ...
    ON_BN_CLICKED(IDC_BUTTON_Health_Mode, &CG910_SAMPLEDlg::OnBnClickedButtonHealthMode)
...
void function_key_change_color(int red, int green){
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::ESC, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::F1, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::F2, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::F3, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::F4, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::F5, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::F6, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::F7, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::F8, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::F9, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::F10, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::F11, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::F12, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::PRINT_SCREEN, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::SCROLL_LOCK, red, green, 0);
    LogiLedSetLightingForKeyWithKeyName(LogiLed::KeyName::PAUSE_BREAK, red, green, 0);
}

void bad_health(int &red, int &green, int &speed){
    red=min(red+(green==0)*speed, 100);
    green=max(green-(red==0)*speed, 0);
}

void good_health(int &red, int &green, int &speed){
    red=max(0, red-(green==0)*speed);
    green=min(100, green+(red==0)*speed);    
}

/* -------------------- */
void CG910_SAMPLEDlg::OnBnClickedButtonHealthMode(){
    LogiLedSetLighting(0, 0, 100);
	int red = 0, green = 100;
    int duration_wait_key = 5, good_speed = 10, bad_speed = 10, led_duration = 1;
    bool is_health = true;
    function_key_change_color(0, 100);
    
    time_t last_hit, now_time, last_change;
    
    last_change = time(nullptr);
    ...
    while(1){
        for(k=10; k<128; k++){
            ...
            // wait at least 3 sec to start decrease the red
            if((long long)now_time-(long long)last_hit > duration_wait_key){
                if(!is_health){
                    is_health = true;
                    last_hit = time(nullptr);
                } 
            ...
            // check key hit
            if((GetAsyncKeyState(k) & 0x8000)) is_health = false;
            

            // each 1 sec, increase/decrease the red
            if((long long)now_time-(long long)last_change >= led_duration){
                last_change = time(nullptr);
                if(is_health) good_health(red, green, good_speed);
                else bad_health(red, green, bad_speed);

                function_key_change_color(red, green);
            }
        }
    }
}
